...
class AAttn(nn.Module): #这个是YOLOv11官方人员改进的
    """
...
##### 更新版C3k2实现
class Dy_C3k2(C2f):  # 假设C2f是已定义的基类
    """优化后的C3k2模块"""

    def __init__(self, c1, c2, n=1, c3k=False, e=0.5, g=4, shortcut=True):
        super().__init__(c1, c2, n, shortcut, g, e)
        self.m = nn.ModuleList(
            [  # 列表推导式的左括号
                self.create_block(self.c, self.c, 2, shortcut, g)
                for _ in range(n)
            ]  # 列表推导式的右括号
        )  # M
    @staticmethod
    def create_block(c_in, c_out, kernel_size, shortcut, groups):
        return nn.Sequential(
            # 深度可分离卷积
            nn.Conv2d(c_in, c_in, kernel_size,
                      padding=(kernel_size - 1) // 2, groups=c_in),
            nn.BatchNorm2d(c_in),
            nn.Conv2d(c_in, c_out, 1),
            DynamicReLU(c_out),
            Shortcut(c_out, c_out) if shortcut else Identity()  # 修正后的调用
        )
...
class DynamicReLU(nn.Module):
    ...
    def __init__(self, channels, alpha=0.5):
        super().__init__()
        self.alpha = nn.Parameter(torch.tensor([alpha] * channels))

    def forward(self, x):
        scale = self.alpha.view(1, -1, 1, 1)
        return torch.max(x, scale * x)
...
# 修改C2f_MMSA模块
class C2f_MMSA(nn.Module):
    def __init__(self, c1, c2, n=2, shortcut=True, g=1, e=0.5):
        super(C2f_MMSA, self).__init__()
        self.c = c2  # 总通道数
        self.cv1 = nn.Conv2d(c1, c2, 1)  # 初始卷积
        # 每个分支包含n个MMSA模块
        self.mhsa_branch1 = nn.ModuleList([MMSA(c2 // 2) for _ in range(n)])
        self.mhsa_branch2 = nn.ModuleList([MMSA(c2 // 2) for _ in range(n)])
        self.cv3 = nn.Conv2d(c2, c2, 1)  # 最终卷积
    ...
    def forward(self, x):
        ...
        # 分支1经过n个MMSA
        for msa in self.mhsa_branch1:
            x1 = msa(x1)
        # 分支2经过n个MMSA
        for msa in self.mhsa_branch2:
            x2 = msa(x2)
        # 拼接并卷积
...
# 基础组件 CBS = Conv + BN + SiLU
class CBS(nn.Module):
    def __init__(self, in_c, out_c, kernel_size=3, stride=1):
        super().__init__()
        self.conv = nn.Conv2d(in_c, out_c, kernel_size, stride, padding=kernel_size // 2, bias=False)
...
class CRU(nn.Module):
    def __init__(self, in_c, out_c):
        super().__init__()
        self.conv = CBS(in_c, out_c, kernel_size=1)
...
class SCRB(nn.Module):
    """Spatial-Channel Residual Block with configurable stacked modules."""
    def __init__(self, c1, c2, n=1, use_cru=True, e=0.5, g=1, shortcut=True):
        """
        Args:
            c1: 输入通道数
            c2: 输出通道数
            n: 堆叠模块的重复次数
            use_cru: 是否使用CRU进行通道调整（默认True）
            e: 通道扩展系数（中间通道数 = c2 * e）
            g: 分组卷积的分组数
            shortcut: 是否启用残差连接
        """
        ...
        self.cru = CRU(c1, c2) if (use_cru and c1 != c2) else nn.Identity()
        self.m = nn.ModuleList(
            nn.Sequential(
                CBS(self.c if i == 0 else c2, c2, kernel_size=3, stride=1),
                CBS(c2, c2, kernel_size=3, stride=1)
            ) for i in range(n)
        )
    ...
    def forward(self, x):
        residual = self.cru(x)
        x = self.m[0](x)  # 第一个模块处理原始输入
        for module in self.m[1:]:  # 后续模块依次处理
            x = module(x)
