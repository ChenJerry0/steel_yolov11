...
__all__=['C3k2_GCSA','C2f_GCSA','C3k2_DTAB','C2f_DTAB',"SFY","SFY1","SFY2","SFY3","SFY4","SFYSelector"]

from ultralytics.nn.modules import C3

# https://arxiv.org/pdf/2404.07846
'''
来自AAAI 2025顶会  哈尔滨工业大学
即插即用模块： DTAB和GCSA  (整合全局与局部特征信息)

盲点网络（BSN）是自监督图像去噪（SSID）中普遍采用的神经架构。然而，大多数现有的BSN都是基于卷积层构建的。
尽管Transformer在许多图像修复任务中显示出克服卷积限制的潜力，但其注意力机制可能会违反盲点要求，从而限制了它们在BSN中的适用性。
...
适用于：图像增强，目标检测，图像分割，图像分类，超分图像任务，语义分割，遥感语义分割等所有CV任务通用即插即用模块
'''
...
class DTAB(nn.Module):
    def __init__(self, dim, window_size=4, overlap_ratio=0.5, num_channel_heads=2, num_spatial_heads=2, spatial_dim_head=16, ffn_expansion_factor=1, bias=False, LayerNorm_type='BiasFree'):
...
class Bottleneck_GCSA(nn.Module):
    """Standard bottleneck."""
...
class C2f_GCSA(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):
        ...
        self.m = nn.ModuleList(Bottleneck_GCSA(self.c, self.c, shortcut, g, k=((3, 3), (3, 3)), e=1.0) for _ in range(n))
...
class C3k_GCSA(C3):
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5, k=3):
        ...
        c_ = int(c2 * e)  # hidden channels
        self.m = nn.Sequential(*(Bottleneck_GCSA(c_, c_, shortcut, g, k=((3, 3), (3, 3)), e=1.0) for _ in range(n)))

class C3k2_GCSA(C2f_GCSA):
    """Faster Implementation of CSP Bottleneck with 2 convolutions."""
    ...
    def __init__(self, c1, c2, n=1, c3k=False, e=0.5, g=1, shortcut=True):
        ...
        self.m = nn.ModuleList(
            C3k_GCSA(self.c, self.c, 2, shortcut, g) if c3k else Bottleneck_GCSA(self.c, self.c, shortcut, g, k=((3, 3), (3, 3)), e=1.0) for _ in range(n)
        )

class C2f_DTAB(nn.Module):
    """Faster Implementation of CSP Bottleneck with 2 convolutions."""
...
class C3k2_DTAB(C2f_GCSA):
    ...
    def __init__(self, c1, c2, n=1, c3k=False, e=0.5, g=1, shortcut=True):
        ...
        super().__init__(c1, c2, n, shortcut, g, e)
        self.m = nn.ModuleList(
            C3k_GCSA(self.c, self.c, 2, shortcut, g) if c3k else DTAB(self.c) for _ in range(n)
        )
...
"""
SFY(Space-Features-Pyramids) 模块，双向金字塔结构，输入为4层特征图（深度由浅至深，大小由大至小）, 13M 
"""
...
class DownBlock(nn.Module):
    def __init__(self, feature_size=128):
        ...
        self.conv1 = nn.Conv2d(feature_size, feature_size, kernel_size=3, stride=1, padding=1)
        ...
        self.conv4 = nn.Conv2d(feature_size * 2, feature_size, kernel_size=1, stride=1, padding=0)

        self.avg_pool = nn.AdaptiveAvgPool2d((1, 1))
        self.max_pool = nn.MaxPool2d(kernel_size=5, stride=1, padding=5 // 2)
    ...
    def forward(self, pre, cur):
        shortcut = cur
        cur = self.conv1(cur)
        pre = F.interpolate(pre, size=cur.size()[2:], mode='bilinear')
        pre = self.conv2(pre)
        down = self.conv3(pre + cur)

        avg_down = self.avg_pool(down)
        max_down = self.max_pool(down)

        avg_down = self.softmax(avg_down) * down
        max_down = self.softmax(max_down) * down

        down = torch.concat([avg_down, max_down], dim=1)

        down = self.bn(self.conv4(down))
...
class UpBlock(nn.Module):
    def __init__(self, feature_size=128):
        ...
        self.conqk = nn.Conv2d(feature_size, 1, kernel_size=1, stride=1, padding=0)
        self.softmax = nn.Softmax(dim=1)
    ...
    def forward(self, cur, nxt):
        shortcut = cur
        cur_h, cur_w = cur.size()[2:]
        nxt = self.conv1(nxt)
        cur = F.interpolate(cur, size=nxt.size()[2:], mode='bilinear')
        cur = self.conv2(cur)

        up = self.conv3(cur + nxt)
        n, c, h, w = up.shape
        v = up.permute(0, 2, 3, 1).reshape(n, h * w, c).unsqueeze(-1) # [N, H * W, C, 1]
        qk = self.conqk(up).permute(0, 2, 3, 1).reshape(n, h * w, 1).unsqueeze(-1) # [N, H * W, 1, 1]
        qk = self.softmax(qk)
        up = torch.einsum('ncht,nctw->nchw', (v, qk)).squeeze(-1) # [N, H * W, C]
        up = up.permute(0, 2, 1).reshape(n, c, h, w)
        up = self.bn(up)
        up = F.interpolate(up, size=(cur_h, cur_w), mode='bilinear')

        return shortcut + up


class SFY1(nn.Module):
    def __init__(self, feature_size=128, dims = [128, 256, 512, 1024]):
        super(SFY1, self).__init__()

        # self.conv3 = nn.Conv2d(feature_size * 2, feature_size, kernel_size=1, stride=1, padding=0)
        self.conv4 = nn.Conv2d(feature_size * 2, feature_size, kernel_size=1, stride=1, padding=0)
        # self.conv5 = nn.Conv2d(feature_size * 2, feature_size, kernel_size=1, stride=1, padding=0)

        self.p2 = nn.Conv2d(dims[0], feature_size, kernel_size=1, stride=1, padding=0)
        self.p3 = nn.Conv2d(dims[1], feature_size, kernel_size=1, stride=1, padding=0)
        self.p4 = nn.Conv2d(dims[2], feature_size, kernel_size=1, stride=1, padding=0)
        self.p5 = nn.Conv2d(dims[3], feature_size, kernel_size=1, stride=1, padding=0)
    ...
    def forward(self, x):
        in2, in3, in4, in5 = self.reform(x)
        out6 = self.p6(in5)


        down5 = self.down1(out6, in5)
        down4 = self.down2(down5, in4)


        up3 = self.up1(in3, in2)
        up4 = self.up2(in4, up3)


        #out3 = self.conv3(torch.concat([up3, down3], dim=1))
        out4 = self.conv4(torch.concat([up4, down4], dim=1))
        #return [out3, out4, out5, out6, out7]
        return out4

class SFY2(nn.Module):
    def __init__(self, feature_size=128, dims = [128, 256, 512, 1024]):
        super(SFY2, self).__init__()
    ...
    def forward(self, x):
        in2, in3, in4, in5 = self.reform(x)
        out6 = self.p6(in5)
        # out7 = F.relu(self.p7(out6))


        down5 = self.down1(out6, in5)
        # down4 = self.down2(down5, in4)
        # down3 = self.down3(down4, in3)


        up3 = self.up1(in3, in2)
        up4 = self.up2(in4, up3)
        up5 = self.up3(in5, up4)


        #out3 = self.conv3(torch.concat([up3, down3], dim=1))
        # out4 = self.conv4(torch.concat([up4, down4], dim=1))
        out5 = self.conv5(torch.concat([up5, down5], dim=1))
        #return [out3, out4, out5, out6, out7]
        return out5

class SFY3(nn.Module):
    def __init__(self, feature_size=128, dims = [128, 256, 512, 1024]):
        super(SFY3, self).__init__()
...
class SFY4(nn.Module):
    def __init__(self, feature_size=128, dims = [128, 256, 512, 1024]):
        super(SFY4, self).__init__()
...
class SFYSelector(nn.Module):
    """
    SFY特征选择模块
    输入: SFY模块输出的四个特征图元组 (out4, out5, out6, out7)
    输出: 根据参数k选择的单个特征图
    """
    ...
    def __init__(self, k=0):
        """
        初始化特征选择器
        :param k: 选择输出的特征图索引 (0:out4, 1:out5, 2:out6, 3:out7)
        """
        super(SFYSelector, self).__init__()
        self.k = k

    def forward(self, features):
        """
        处理SFY输出的特征图
        :param features: 包含四个特征图的元组 (out4, out5, out6, out7)
        :return: 选择的特征图
        """
        # 确保输入是包含四个特征图的元组
        features = features[0]
        if not isinstance(features, tuple) or len(features) != 4:
            raise ValueError(
                f"输入必须是包含4个特征图的元组，实际收到: {type(features)} (长度: {len(features) if isinstance(features, tuple) else 'N/A'})")
...
if __name__ == "__main__":
    ...
    input_tensor1 = torch.randn(1,128,256,256)
    ...
    in2 = torch.randn(1, 128, 128, 128)  # 浅层特征 (大尺寸)
    in3 = torch.randn(1, 256, 64, 64)
    in4 = torch.randn(1, 512, 32, 32)
    in5 = torch.randn(1, 1024, 16, 16)  # 深层特征 (小尺寸)

    # 初始化SFY模块（保持默认参数）
    sfy_module = SFY( )

    # 前向传播
    outputs = sfy_module([in2, in3, in4, in5])
    outputs1 = SFYSelector(1)
    # 打印输入输出形状
    print("SFY测试结果：")
    print("输入形状:")
    print(f"in2: {in2.shape} (应匹配dims[0]=256)")
    print(f"in3: {in3.shape} (应匹配dims[1]=512)")
    print(f"in4: {in4.shape} (应匹配dims[2]=1024)")
    print(f"in5: {in5.shape} (应匹配dims[3]=2048)")

    # print("\n输出形状:", outputs.shape)
    for i, out in enumerate(outputs):
        print(f"out{i + 4}: {out.shape} (通道数应保持256)")
