...
'''
# Register your custom modules with Ultralytics
from ultralytics.nn.modules import C2f


# Copy the LayerNorm, DConv7, MLP, CID, and MCC implementations from the previous artifact
...
class GFM(nn.Module):
    def __init__(self, in_channels, feature_num=2, bias=True, padding_mode='reflect', **kwargs) -> None:
        ...
        self.cld = CID(in_channels,padding_mode)
...
if __name__ == '__main__':
    ...
    # 创建一个输入特征图
    input_tensor = torch.randn(2, 64, 32, 32)  # (batch_size, channels, height, width)
    # 创建 MCC 实例（参数需与输入通道数匹配）
    MCC_module = MCC(f_number=64,num_heads=4,padding_mode='zeros')
    # 前向传播
    output = MCC_module(input_tensor)
    ...
    # 创建一个输入特征图
    input_tensor = torch.randn(2, 64, 32, 32)  # (batch_size, channels, height, width)
    # 创建 CID 实例（参数需与输入通道数匹配）
    CID_module = CID(
        f_number=64,padding_mode='zeros'
    )
    # 前向传播
    output = CID_module(input_tensor)
