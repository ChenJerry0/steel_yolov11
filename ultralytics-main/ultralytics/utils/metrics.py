...
class WIoU_Scale:
    """Wise-IoU缩放系数计算（带稳定性改进）"""

    def __init__(self, iou, momentum=0.9, max_scale=2.0):  # 添加默认参数
        self.momentum = momentum
        self.max_scale = max_scale
        self.iou_mean = iou.detach().mean().item()
        self._scale = torch.ones_like(iou)

    def update(self, iou):
        new_mean = iou.detach().mean().item()
        self.iou_mean = self.momentum * self.iou_mean + (1 - self.momentum) * new_mean
        return self

    @property
    def _scaled_loss(self):
        scale = (self.iou_mean / (self._scale + 1e-7)).sqrt()
        return torch.clamp(scale, 0.5, self.max_scale)  # 使用类内参数
...
def bbox_iou(box1, box2, xywh=True, GIoU=False, DIoU=False, CIoU=False, SIoU=False, EIoU=False, WIoU=False, Focal=False,
             alpha=1, gamma=0.5, scale=False, eps=1e-7, momentum=0.9, max_scale=2.0):
    ...
    if scale:
        # 正确传递参数到WIoU_Scale
        self_scale = WIoU_Scale(1 - (inter/union), momentum=momentum, max_scale=max_scale)
    ...
    with torch.no_grad():
        ...
        c2 = cw**2 + ch**2 + eps
        rho2 = torch.clamp(rho2, max=20.0)  # 限制中心距离平方范围
        c2 = torch.clamp(c2, min=1e-6)     # 防止分母过小
    ...
    if CIoU or DIoU or GIoU or EIoU or SIoU or WIoU:
        ...
        if CIoU or DIoU or EIoU or SIoU or WIoU:  # Distance or Complete IoU https://arxiv.org/abs/1911.08287v1
            ...
            if CIoU:  # https://github.com/Zzh-tju/DIoU-SSD-pytorch/blob/master/utils/box/box_utils.py#L47
                ...
                if Focal:
                    raise RuntimeError("WIoU暂不支持Focal Loss")
                # 动态缩放系数
                if scale:
                    scale_factor = self_scale._scaled_loss  # 使用已初始化的参数
                    loss = scale_factor * (1 - iou) * torch.exp(rho2 / c2.detach())
                else:
                    loss = (1 - iou) * torch.exp(rho2 / c2.detach())
...
def new_bbox_iou(box1, box2, xywh=True, GIoU=False, DIoU=False, CIoU=False, SIoU=False, EIoU=False, WIoU=False,
                 ...
                 Focal=False, alpha=1, gamma=0.5, scale=False, eps=1e-7, momentum=0.9, max_scale=2.0,
    ...
    # 增加梯度裁剪保护
    cw = b1_x2.maximum(b2_x2) - b1_x1.minimum(b2_x1)  # 包围框宽度
    ch = b1_y2.maximum(b2_y2) - b1_y1.minimum(b2_y1)  # 包围框高度
    ...
    with torch.no_grad():
        ...
        # 数值稳定性处理
        rho2 = torch.clamp(rho2, max=20.0)  # 扩大上限防止梯度爆炸
        c2 = torch.clamp(c2, min=1e-6)  # 防止除零错误
    ...
    union = w1 * h1 + w2 * h2 - inter + eps
    if scale:
        self_scale = WIoU_Scale(1 - (inter/union),
                              momentum=momentum,     # 传递参数
                              max_scale=max_scale)
