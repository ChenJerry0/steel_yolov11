...
# Load a model
model = YOLO("yolo11n-pose.pt.yaml")  # build a new model from YAML
model = YOLO("yolo11n-pose.pt.pt")  # load a pretrained model (recommended for training)
...
# Load a model
model = YOLO("yolo11n-pose.pt.pt")  # load an official model
model = YOLO("path/to/best.pt")  # load a custom model
